...
const post = SEED_BLOG_POSTS.find((p) => p.slug === "best-coworking-spaces-in-lucknow-2026");
if (!post) {
  console.error("Seed post not found");
  process.exit(1);
}
...
async function main() {
  ...
  if (existing?.id) {
    ...
    console.log("Updated published post:", post.slug);
  } else {
    ...
    console.log("Inserted published post:", post.slug);
  }
